...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'sync') {
        // Sync entire cart for a table (replaces existing cart for that table)
        ...
        if (!empty($table_id)) {
            ...
            if (!empty($cartItems)) {
                ...
                foreach($cartItems as $item) {
                    ...
                    $optionsJson = isset($item['ingredients']) && is_array($item['ingredients']) ? json_encode($item['ingredients'], JSON_UNESCAPED_UNICODE) : '{}';
